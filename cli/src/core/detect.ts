...
export function candidateToCommand(c: Candidate): Suggestion {
  ...
  const sequenceSteps = c.kind === "sequence" ? safeSignature.split(/\s+→\s+/) : [];
  const trigger = sequenceSteps[0] ?? safeSignature;
  const words = trigger.split(" ").slice(0, 3).join(" ");
  ...
  return {
    ...
    payload: {
      ...
      body: c.kind === "paste"
        ...
        : c.kind === "sequence"
          ? sequenceSteps.map((step, index) => `${index + 1}. ${step}`).join("\n")
...
export function buildDetectPrompt(cands: Candidate[]): { system: string; prompt: string } {
  const system =
    "You convert clusters of a developer's repeated coding-assistant prompts into reusable artifacts. " +
    ...
    "Also set mechanical:true only when executing the workflow requires zero judgment calls (for example, a fixed retarget-and-retry git push sequence). " +
    "Leave mechanical false or absent when the steps require judgment (for example, review a spec and then write an implementation plan). " +
