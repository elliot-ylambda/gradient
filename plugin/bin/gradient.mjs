...
var require_url_state_machine = __commonJS({
  "node_modules/whatwg-url/lib/url-state-machine.js"(exports, module) {
    ...
    URLStateMachine.prototype["parse file"] = function parseFile2(c3) {
  ...
  return (function(e3) {
    ...
    function g2() {
      l2 && (i2.releaseLock(), i2 = e3.getReader(), y2(i2), l2 = false), b(i2.read(), ((e4) => {
        ...
        return h2 || o2.enqueue(u2), p2 || n2.enqueue(c3), s2 = false, d2 ? S2() : f2 && v2(), null;
      }), (() => (s2 = false, null)));
    }
    function w2(t4, r3) {
      ...
      b(i2.read(t4), ((e4) => {
        ...
        if (e4.done) {
          o3 || u2.close(), n3 || c3.close();
          const r4 = e4.value;
          return void 0 !== r4 && (o3 || u2.byobRequest.respondWithNewView(r4), n3 || null === (t5 = c3.byobRequest) || void 0 === t5 || t5.respond(0)), o3 && n3 || a2(void 0), null;
        }
        const l3 = e4.value;
        if (n3) o3 || u2.byobRequest.respondWithNewView(l3);
          ...
          try {
            ...
            return u2.error(e6), c3.error(e6), a2(i2.cancel(e6)), null;
          }
          o3 || u2.byobRequest.respondWithNewView(l3), c3.enqueue(e5);
        }
...
async function fileFromPath2(path, filenameOrOptions, options) {
  const stats2 = await fs.stat(path);
  return createFileFromPath(path, stats2, filenameOrOptions, options);
}
...
async function walk(dir) {
  ...
  for (const e2 of entries) {
    const full = join(dir, e2.name);
    if (e2.isDirectory()) {
      if (e2.name === "subagents") continue;
      out.push(...await walk(full));
    } else if (e2.name.endsWith(".jsonl")) {
      out.push(full);
    }
  }
  return out;
...
async function collect(opts) {
  ...
  for (const f2 of files) {
    const s2 = await stat(f2);
    if (matchesSince(s2.mtimeMs, opts.sinceDays, now)) kept.push(f2);
  }
  return kept;
}
...
function parseOne(line) {
  ...
  if (typeof content === "string") text = content;
    ...
    const parts = content.filter((b2) => b2.type === "text").map((b2) => b2.text ?? "");
...
function isInjected(text) {
  const t2 = text.trim();
  if (!t2) return true;
  return INJECTED_PATTERNS.some((re2) => re2.test(t2));
}
function filterPrompts(turns) {
  return turns.filter((t2) => t2.role === "user" && t2.text !== void 0 && !isInjected(t2.text));
}

// src/core/cap.ts
function capByRecency(prompts, max) {
  if (max <= 0 || prompts.length <= max) return { kept: prompts, dropped: 0 };
  const sorted = [...prompts].sort((a2, b2) => a2.ts < b2.ts ? 1 : a2.ts > b2.ts ? -1 : 0);
  return { kept: sorted.slice(0, max), dropped: prompts.length - max };
}

// src/core/scope.ts
var DEFAULT_USER_SCOPE_DAYS = 7;
var DEFAULT_MAX_PROMPTS = 1500;
var DEFAULT_DETECT_WINDOW = 24;
...
function coeffs(n2) {
  const a2 = [], b2 = [];
  for (let i2 = 0; i2 < n2; i2++) {
    a2.push(Math.imul(i2, 2) + 1 >>> 0 | 1);
    b2.push(Math.imul(i2, 2246822519) + 374761393 >>> 0);
  }
  return { a: a2, b: b2 };
}
function minhash(shingles, numHashes = LSH_NUM_HASHES) {
  const { a: a2, b: b2 } = coeffs(numHashes);
  const out = new Array(numHashes).fill(4294967295);
  for (const sh of shingles) {
    const x2 = h32(sh);
    for (let i2 = 0; i2 < numHashes; i2++) {
      const v2 = Math.imul(a2[i2], x2) + b2[i2] >>> 0;
      if (v2 < out[i2]) out[i2] = v2;
    }
  }
  return out;
}
...
function normalize(s2) {
  return s2.toLowerCase().trim().replace(/\s+/g, " ").replace(/[.!?,;:]+$/g, "").trim();
}
function trigrams(s2) {
  const padded = `  ${s2} `;
  const out = /* @__PURE__ */ new Set();
  for (let i2 = 0; i2 < padded.length - 2; i2++) out.add(padded.slice(i2, i2 + 3));
  return out;
}
function similarity(a2, b2) {
  if (a2 === b2) return 1;
  const ta = trigrams(a2), tb = trigrams(b2);
  let inter = 0;
  for (const g2 of ta) if (tb.has(g2)) inter++;
...
function cluster(turns, opts = {}) {
  ...
  for (const t2 of turns) {
    if (t2.role !== "user" || !t2.text) continue;
    const norm = normalize(t2.text);
    if (norm.length < 2) continue;
    let b2 = exact.get(norm);
    if (!b2) {
      b2 = { signature: norm, examples: [], count: 0, sessions: /* @__PURE__ */ new Set() };
      exact.set(norm, b2);
    }
    b2.count++;
    b2.sessions.add(t2.sessionId);
    if (b2.examples.length < 5) b2.examples.push(t2.text);
  }
  const buckets = [...exact.values()].sort((a2, b2) => b2.count - a2.count);
  const merged = [];
  const fuzzyMember = [];
  const bandIndex = /* @__PURE__ */ new Map();
  for (const b2 of buckets) {
    const keys = bandKeys(minhash(trigrams(b2.signature)));
    const candidateHosts = /* @__PURE__ */ new Set();
    for (const k2 of keys) for (const hi of bandIndex.get(k2) ?? []) candidateHosts.add(hi);
    let hostIdx = -1;
    for (const hi of [...candidateHosts].sort((x2, y2) => x2 - y2)) {
      if (similarity(merged[hi].signature, b2.signature) >= simThreshold) {
        hostIdx = hi;
        break;
      }
    }
    if (hostIdx >= 0) {
      ...
      host.count += b2.count;
      for (const s2 of b2.sessions) host.sessions.add(s2);
      for (const ex of b2.examples) if (host.examples.length < 5) host.examples.push(ex);
      fuzzyMember[hostIdx] = true;
    } else {
      merged.push({ ...b2, sessions: new Set(b2.sessions) });
      const idx = merged.length - 1;
      fuzzyMember[idx] = false;
      for (const k2 of keys) {
        const arr = bandIndex.get(k2) ?? [];
        arr.push(idx);
        bandIndex.set(k2, arr);
      }
    }
  }
  const candidates = [];
  merged.forEach((b2, i2) => {
    if (b2.count < minCount) return;
    const confidence = fuzzyMember[i2] ? "inferred" : "high";
    candidates.push({
      kind: "unknown",
      signature: b2.signature,
      examples: b2.examples,
      count: b2.count,
      sessions: b2.sessions.size,
      sessionIds: [...b2.sessions],
      confidence
    });
  });
  return candidates.sort((a2, b2) => b2.count - a2.count);
}
...
function assertInside(base, target) {
  const b2 = resolve(base);
  const t2 = resolve(target);
  const rel = relative(b2, t2);
  if (rel.startsWith("..") || isAbsolute(rel)) {
    throw new Error(`refusing to write outside ${b2}: ${t2}`);
  }
}
...
function redact(text) {
  let out = text;
  for (const re2 of SECRET_PATTERNS) out = out.replace(re2, "[REDACTED]");
  return out;
}

// src/core/detect.ts
var ALLOWED_CONFIDENCE = /* @__PURE__ */ new Set(["high", "inferred", "flagged"]);
function idFor(signature) {
  return createHash("sha1").update(signature).digest("hex").slice(0, 10);
}
function candidateToCommand(c3) {
  const words = c3.signature.split(" ").slice(0, 3).join(" ");
  const commandName = sanitizeName(words);
  return {
    id: idFor(c3.signature),
    name: commandName,
    title: `Reusable command for "${c3.signature}"`,
    rationale: `Repeated ${c3.count}\xD7 across ${c3.sessions} sessions.`,
    evidence: { count: c3.count, sessions: c3.sessions },
    confidence: c3.confidence,
    examples: c3.examples.map(redact).slice(0, 5),
    payload: { type: "command", commandName, body: c3.examples[0] ?? c3.signature }
  };
}
function degradeToCommands(cands) {
  return cands.filter((c3) => c3.confidence === "high").map(candidateToCommand);
}
function buildDetectPrompt(cands) {
  const system = `You convert clusters of a developer's repeated Claude Code prompts into reusable artifacts. For each cluster decide a type: 'command' (a repeated instruction \u2192 slash command), 'loop' (a recurring cadence task), or 'hook' (an automation tied to a Claude Code lifecycle event; the only supported hook event is PreCompact backed by the gradient subcommand 'checkpoint'). Merge clusters that mean the same thing (e.g. 'lgtm' and 'looks good') into ONE suggestion. Echo back EVERY merged cluster's exact 'signature' in a 'sourceSignatures' string array so evidence can be summed. Respond ONLY with JSON: {"suggestions":[{sourceSignatures,name,title,rationale,confidence,payload}]} where payload is one of {type:'command',commandName,body} | {type:'loop',instruction,cadence?} | {type:'hook',event:'PreCompact',subcommand:'checkpoint',description}. confidence must be exactly one of "high", "inferred", or "flagged".`;
  const prompt = JSON.stringify(
    cands.map((c3) => ({
      signature: redact(c3.signature),
      count: c3.count,
      sessions: c3.sessions,
      examples: c3.examples.map(redact),
      confidence: c3.confidence
    })),
    null,
    2
...
async function detect(cands, llm, opts = {}) {
  ...
  const ranked = [...cands].sort((a2, b2) => b2.count - a2.count);
  ...
  try {
    ...
    const bySignature = new Map(top.map((c3) => [redact(c3.signature), c3]));
    return (parsed.suggestions ?? []).filter((s2) => !!s2 && !!s2.payload && typeof s2.payload.type === "string").map((s2) => {
      const sigs = s2.sourceSignatures ?? (s2.sourceSignature ? [s2.sourceSignature] : []);
      const matched = sigs.map((sig) => bySignature.get(redact(sig))).filter((c3) => !!c3);
      const count = matched.reduce((n2, c3) => n2 + c3.count, 0);
      const sessions = new Set(matched.flatMap((c3) => c3.sessionIds)).size;
      const examples = matched.flatMap((c3) => c3.examples).map(redact).slice(0, 5);
      return {
        id: idFor(s2.payload.type === "command" ? s2.payload.commandName ?? s2.name : s2.name),
        name: s2.name,
        title: s2.title,
        rationale: s2.rationale,
        evidence: { count, sessions },
        confidence: ALLOWED_CONFIDENCE.has(s2.confidence) ? s2.confidence : "inferred",
        examples,
        payload: s2.payload
      };
    });
...
var KNOWN_SUBCOMMANDS = /* @__PURE__ */ new Set(["checkpoint", "scan"]);
var TYPES = /* @__PURE__ */ new Set(["command", "loop", "hook"]);
var CONFIDENCES = /* @__PURE__ */ new Set(["high", "inferred", "flagged"]);
function validateSuggestion(x2) {
  const s2 = x2;
  if (!s2 || typeof s2 !== "object") throw new Error("suggestion is not an object");
  for (const k2 of ["id", "name", "title", "rationale", "confidence"]) {
    if (typeof s2[k2] !== "string") throw new Error(`suggestion.${k2} must be a string`);
  }
  if (!CONFIDENCES.has(s2.confidence)) {
    throw new Error(`invalid confidence: ${String(s2.confidence)}`);
  }
  const payload = s2.payload;
...
function assertHookRunnable(s2) {
  if (s2.payload.type !== "hook") return;
  if (!KNOWN_SUBCOMMANDS.has(s2.payload.subcommand)) {
    throw new Error(`hook references unknown gradient subcommand: ${s2.payload.subcommand}`);
  }
}
...
async function loadManifest(projectDir) {
  let raw;
  try {
    raw = await readFile2(manifestPath(projectDir), "utf8");
  } catch {
    return [];
  }
...
async function addEntry(projectDir, e2) {
  const entries = (await loadManifest(projectDir)).filter((x2) => x2.name !== e2.name);
  entries.push(e2);
  await save(projectDir, entries);
}
async function removeEntry(projectDir, name) {
  const entries = await loadManifest(projectDir);
  const found = entries.find((x2) => x2.name === name);
  if (!found) return void 0;
  await save(projectDir, entries.filter((x2) => x2.name !== name));
  return found;
}
...
var DEFAULT_PLAYBOOK = `# gradient.md \u2014 autopilot playbook
...
function playbookPath(home) {
  return join3(home ?? homedir2(), ".config", "gradient", "gradient.md");
}
function isNudge(s2) {
  return s2.payload.type === "loop" && !s2.payload.cadence;
}
function renderMinedSection(suggestions) {
  const nudgeLines = suggestions.filter(isNudge).map((s2) => s2.payload.type === "loop" ? `- "${s2.payload.instruction}" (seen ${s2.evidence.count}\xD7 \xB7 ${s2.evidence.sessions} sessions)` : "").filter(Boolean);
  const cmdLines = suggestions.filter((s2) => s2.payload.type === "command").map((s2) => `- /${s2.name} \u2014 ${s2.title}`);
  return [
    "## How I nudge (mined)",
    "",
...
function generatePlaybook(suggestions, existing) {
...
async function writePlaybook(suggestions, home) {
  ...
  await writeFile2(path, next);
  return path;
}
async function loadPlaybook(home) {
  try {
    return await readFile3(playbookPath(home), "utf8");
  } catch {
    return DEFAULT_PLAYBOOK;
  }
}
var MODE_RANK = { off: 0, nudge: 1, full: 2 };
function clampMode(a2, b2) {
  return MODE_RANK[a2] <= MODE_RANK[b2] ? a2 : b2;
}
function projectPlaybookPath(cwd) {
  return join3(cwd, "gradient.md");
}
var isMode = (v2) => v2 === "off" || v2 === "nudge" || v2 === "full";
function parseProjectPlaybook(raw) {
  ...
  for (let i2 = 1; i2 < end; i2++) {
    const modeM = lines[i2].match(/^\s*max-mode\s*:(.*)$/i);
    if (modeM) {
      const v2 = modeM[1].trim();
      if (!isMode(v2)) return malformed();
      clamps.maxMode = v2;
      continue;
    }
    const budgetM = lines[i2].match(/^\s*budget\s*:(.*)$/i);
    if (budgetM) {
      const v2 = budgetM[1].trim();
      const n2 = Number(v2);
      if (v2 === "" || !Number.isInteger(n2) || n2 < 0) return malformed();
      clamps.budget = n2;
    }
  }
...
async function loadProjectPlaybook(cwd) {
  try {
    return parseProjectPlaybook(await readFile3(projectPlaybookPath(cwd), "utf8"));
  } catch (e2) {
    const err = e2;
...
var defaultRun = (cmd, args, input, opts) => new Promise((resolveP) => {
...
var defaultWhich = (bin) => new Promise((resolveP) => {
  const child = spawn(process.platform === "win32" ? "where" : "which", [bin]);
  let out = "";
  child.stdout.on("data", (d2) => out += d2);
...
var AnthropicBackend = class {
  ...
  async complete(req) {
    const client = new sdk_default({ apiKey: this.apiKey });
    ...
    return resp.content.filter((b2) => b2.type === "text").map((b2) => b2.text).join("");
  }
};
...
async function selectBackend(deps = {}) {
  ...
  if (deps.config?.backend) {
    const chosen = candidates.find((c3) => c3.name === deps.config.backend);
    if (chosen && await chosen.available()) return chosen;
  }
  for (const c3 of candidates) {
    if (await c3.available()) return c3;
  }
  return null;
}
...
function configPath(home) {
  return join4(home ?? homedir3(), ".config", "gradient", "config.json");
}
async function loadConfig(home) {
  try {
    return JSON.parse(await readFile4(configPath(home), "utf8"));
  } catch {
    return {};
  }
}
async function saveConfig(c3, home) {
  const p2 = configPath(home);
  await mkdir3(dirname2(p2), { recursive: true });
  await writeFile3(p2, JSON.stringify(c3, null, 2));
}
var DEFAULT_AUTOPILOT_BUDGET = 10;
var DEFAULT_AUTOPILOT_MODEL = "haiku";

// src/commands/scan.ts
async function scan(opts, deps = {}) {
  const log = deps.log ?? (() => {
  });
  const collectFn = deps.collectFn ?? ((o2) => collect(o2));
  const parseFn = deps.parseFn ?? parseFile;
  const files = await collectFn(opts);
  log(`files: ${files.length} transcripts`);
  const turns = [];
  for (const f2 of files) turns.push(...await parseFn(f2));
  ...
  const window2 = opts.limit ?? DEFAULT_DETECT_WINDOW;
  log(`clustering \u2192 ${candidates.length} candidate patterns; sending top ${window2} to llm`);
  ...
  const suggestions = await detect(candidates, backend, {
    limit: window2,
    onCap: (dropped2) => log(`capped to top ${window2}; ${dropped2} lower-frequency candidates dropped`)
  });
  const valid = [];
  for (const s2 of suggestions) {
    try {
      validateSuggestion(s2);
      valid.push(s2);
    } catch (e2) {
      log(`skipping invalid suggestion: ${e2.message}`);
    }
  }
  const projectDir = opts.projectPath ?? process.cwd();
  const gdir = gradientDir(projectDir);
  await mkdir4(gdir, { recursive: true });
  await writeFile4(join5(gdir, "suggestions.json"), JSON.stringify(valid, null, 2));
  ...
  try {
    ...
    log(`gradient.md update failed: ${e2.message}`);
  }
  return valid;
}
...
function emitCommand(s2) {
  if (s2.payload.type !== "command") throw new Error("emitCommand needs a command payload");
  const name = sanitizeName(s2.payload.commandName);
  const description = JSON.stringify(s2.title.replace(/[\r\n]+/g, " ").trim());
  const content = `---
...
function emitLoop(s2) {
  if (s2.payload.type !== "loop") throw new Error("emitLoop needs a loop payload");
  const instruction = s2.payload.instruction.replace(/[\r\n]+/g, " ").replace(/"/g, '\\"').trim();
  const verb = s2.payload.cadence ? "/schedule" : "/loop";
  const cadence = s2.payload.cadence ? `${s2.payload.cadence.replace(/[^A-Za-z0-9 */,:-]/g, "").trim()} ` : "";
...
function emitHook(s2) {
  if (s2.payload.type !== "hook") throw new Error("emitHook needs a hook payload");
  assertHookRunnable(s2);
  if (!KNOWN_HOOK_EVENTS.has(s2.payload.event)) {
    throw new Error(`unknown hook event: ${s2.payload.event}`);
  }
  const patch = {
    hooks: {
      [s2.payload.event]: [
        { hooks: [{ type: "command", command: `gradient ${s2.payload.subcommand}` }] }
      ]
    }
  };
...
function emit(s2) {
  switch (s2.payload.type) {
    case "command":
      return { kind: "command", ...emitCommand(s2) };
    case "loop":
      return { kind: "loop", ...emitLoop(s2) };
    case "hook":
      return { kind: "hook", ...emitHook(s2) };
  }
}

// src/core/apply.ts
async function applySuggestion(s2, projectDir) {
  const result = emit(s2);
  const type = s2.payload.type;
  let written;
  let printed;
  if (result.kind === "command") {
    const abs = join6(projectDir, result.path);
    assertInside(join6(projectDir, ".claude"), abs);
    await mkdir5(dirname3(abs), { recursive: true });
    await writeFile5(abs, result.content);
    written = abs;
  ...
  return { suggestion: s2, written, printed };
}

// src/commands/apply.ts
import { readFile as readFile5 } from "node:fs/promises";
import { join as join7 } from "node:path";
async function loadSuggestions(projectDir) {
  try {
    return JSON.parse(await readFile5(join7(gradientDir(projectDir), "suggestions.json"), "utf8"));
  } catch {
    return [];
  }
}
async function applyByIds(ids, projectDir) {
  const all = await loadSuggestions(projectDir);
  const wanted = all.filter((s2) => ids.includes(s2.id) || ids.includes(s2.name));
  const out = [];
  for (const s2 of wanted) out.push(await applySuggestion(s2, projectDir));
  return out;
}

// src/commands/review.ts
async function review(projectDir, prompt) {
  const suggestions = await loadSuggestions(projectDir);
  const out = [];
  for (let i2 = 0; i2 < suggestions.length; i2++) {
    const decision = await prompt(suggestions[i2], i2, suggestions.length);
    if (decision === "quit") break;
    if (decision === "approve") out.push(await applySuggestion(suggestions[i2], projectDir));
  }
  return out;
}
function readlinePrompter() {
  return async (s2, index, total) => {
    const rl = createInterface({ input: process.stdin, output: process.stdout });
    const label = s2.payload.type;
    process.stdout.write(
      `
(${index + 1}/${total})  ${s2.name} \xB7 ${label} \xB7 seen ${s2.evidence.count}\xD7 \xB7 ${s2.confidence}
  ${s2.title}
`
    );
    if (isNudge(s2)) {
...
async function reviewJson(projectDir) {
  try {
...
async function remove(projectDir, name) {
  const entry = await removeEntry(projectDir, name);
  if (!entry) return false;
  if (entry.path) {
    const abs = isAbsolute2(entry.path) ? entry.path : join8(projectDir, entry.path);
    assertInside(join8(projectDir, ".claude"), abs);
    try {
      await unlink(abs);
    } catch {
    }
...
import { readFile as readFile7, writeFile as writeFile7, mkdir as mkdir7 } from "node:fs/promises";
...
function settingsPath(projectDir) {
  return join9(projectDir, ".claude", "settings.json");
}
function mergeHookIntoSettings(existing, event, command, opts = {}) {
  ...
  const already = groups.some((g2) => g2.hooks?.some((h2) => h2.command === command));
  if (!already) {
...
function removeHookFromSettings(existing, event, command) {
  const out = { ...existing, hooks: { ...existing.hooks ?? {} } };
  const groups = Array.isArray(out.hooks[event]) ? out.hooks[event] : [];
  const kept = groups.map((g2) => ({ ...g2, hooks: (g2.hooks ?? []).filter((h2) => h2.command !== command) })).filter((g2) => g2.hooks.length > 0);
...
async function installHook(projectDir, event, command, opts = {}) {
  ...
  assertInside(join9(projectDir, ".claude"), path);
  let existing = {};
  try {
    existing = JSON.parse(await readFile6(path, "utf8"));
  } catch (e2) {
    if (e2.code !== "ENOENT") {
      throw new Error(`refusing to overwrite unreadable ${path}: ${e2.message}`);
    }
  }
  const merged = mergeHookIntoSettings(existing, event, command, opts);
  await mkdir6(dirname4(path), { recursive: true });
  await writeFile6(path, JSON.stringify(merged, null, 2));
  return path;
}
async function removeHook(projectDir, event, command) {
  const path = settingsPath(projectDir);
  assertInside(join9(projectDir, ".claude"), path);
  let existing;
  try {
...
async function hookInstalled(projectDir, event, command) {
  try {
    const parsed = JSON.parse(await readFile6(settingsPath(projectDir), "utf8"));
    const groups = Array.isArray(parsed?.hooks?.[event]) ? parsed.hooks[event] : [];
    return groups.some((g2) => g2.hooks?.some((h2) => h2.command === command));
  } catch {
    return false;
  }
}
...
async function defaultSkillSource() {
  ...
  return readFile7(join10(here, "..", "..", "src", "skill", "SKILL.md"), "utf8");
}
async function init2(opts, deps = {}) {
  const home = opts.home ?? homedir4();
  const backend = deps.backend !== void 0 ? deps.backend : await selectBackend();
  ...
  if (opts.installSkill) {
    ...
    const dest = join10(home, ".claude", "skills", "gradient", "SKILL.md");
    await mkdir7(dirname5(dest), { recursive: true });
    await writeFile7(dest, source);
    skillInstalled = true;
  }
  ...
  return { backend: backendName, configPath: join10(home, ".config/gradient/config.json"), skillInstalled, sessionScanInstalled };
}

// src/commands/checkpoint.ts
import { writeFile as writeFile8 } from "node:fs/promises";
import { join as join11 } from "node:path";
async function checkpoint(input, projectDir, parseFn = parseFile) {
  const turns = input.transcript_path ? await parseFn(input.transcript_path) : [];
  const prompts = filterPrompts(turns).slice(-10);
  const lines = prompts.map((p2) => `- ${p2.text}`).join("\n");
...
${lines}
`;
  const path = join11(projectDir, "progress.md");
  await writeFile8(path, md);
  return path;
}
...
async function stats(projectDir, opts = {}) {
  ...
  const coveredIds = new Set(manifest.map((m2) => m2.suggestionId));
  const config = await loadConfig(opts.home);
  const patterns = suggestions.map((s2) => ({
    name: s2.name,
    count: s2.evidence.count,
    sessions: s2.evidence.sessions,
    confidence: s2.confidence,
    covered: coveredIds.has(s2.id)
  })).sort((a2, b2) => b2.count - a2.count);
  const total = patterns.length;
  const covered = patterns.filter((p2) => p2.covered).length;
...
async function explain(projectDir, idOrName) {
  const all = await loadSuggestions(projectDir);
  return all.find((s2) => s2.id === idOrName || s2.name === idOrName);
}
...
// src/core/state.ts
import { readFile as readFile8, writeFile as writeFile9, mkdir as mkdir8, readdir as readdir2, stat as stat2, unlink as unlink2 } from "node:fs/promises";
import { join as join12 } from "node:path";
import { homedir as homedir5 } from "node:os";
var LOG_CAP = 20;
var STALE_MS = 7 * 24 * 3600 * 1e3;
function stateDir(home) {
  return join12(home ?? homedir5(), ".config", "gradient", "state");
}
function freshState() {
...
function fileFor(sessionId, home) {
  ...
  return join12(stateDir(home), `${safe}.json`);
}
async function loadState(sessionId, home) {
  try {
    const raw = JSON.parse(await readFile8(fileFor(sessionId, home), "utf8"));
...
async function saveState(sessionId, s2, home) {
  await mkdir8(stateDir(home), { recursive: true });
  const capped = { ...s2, log: s2.log.slice(-LOG_CAP) };
  await writeFile9(fileFor(sessionId, home), JSON.stringify(capped, null, 2));
}
async function cleanupStale(home, now = Date.now()) {
  try {
    const dir = stateDir(home);
    for (const f2 of await readdir2(dir)) {
      try {
        const st2 = await stat2(join12(dir, f2));
        if (now - st2.mtimeMs > STALE_MS) await unlink2(join12(dir, f2));
      } catch {
      }
    }
  } catch {
...
async function latestState(home) {
  try {
    ...
    for (const f2 of await readdir2(dir)) {
      ...
      const st2 = await stat2(join12(dir, f2));
      if (!best || st2.mtimeMs > best.mtime) best = { sessionId: f2.slice(0, -5), mtime: st2.mtimeMs };
    }
    if (!best) return null;
...
var TAIL_MAX_TURNS = 30;
var TAIL_MAX_CHARS = 8e3;
...
function summarizeTools(tools) {
  ...
  for (const t2 of tools) counts.set(t2.name ?? "?", (counts.get(t2.name ?? "?") ?? 0) + 1);
  return [...counts].map(([name, n2]) => n2 > 1 ? `${name} \xD7${n2}` : name).join(", ");
}
  ...
  for (const line of lines) {
    ...
    if (raw.type === "user") {
      ...
      if (typeof content === "string") text = content;
        ...
        text = content.filter((b2) => b2.type === "text").map((b2) => b2.text ?? "").join(" ");
      }
      if (text.trim()) turns.push(`user: ${text.trim()}`);
    } else if (raw.type === "assistant" && Array.isArray(content)) {
      const text = content.filter((b2) => b2.type === "text").map((b2) => b2.text ?? "").join(" ").trim();
      const tools = content.filter((b2) => b2.type === "tool_use");
...
function fingerprint(lines) {
  ...
  for (const line of lines) {
    ...
    if (Array.isArray(content)) {
      for (const b2 of content) if (b2.type === "tool_use") toolUses++;
    }
  }
  return `tools:${toolUses}`;
}
async function readTranscriptLines(path) {
  return (await readFile9(path, "utf8")).split(/\r?\n/);
}
...
var RESPOND_HOOK_COMMAND = "gradient respond";
var HOOK_TIMEOUT_S = 60;
var STATUS_RECENT = 5;
...
async function autopilotStatus(projectDir, opts = {}) {
  ...
  const mode = config.autopilot ?? "off";
...
// src/core/ui.ts
var COLOR = !!process.stdout.isTTY && process.env.NO_COLOR === void 0 && process.env.TERM !== "dumb";
function wrap(open, s2) {
  return COLOR ? `\x1B[${open}m${s2}\x1B[0m` : s2;
}
function rgb(r2, g2, b2, s2) {
  return COLOR ? `\x1B[38;2;${r2};${g2};${b2}m${s2}\x1B[0m` : s2;
}
var G1 = [124, 108, 255];
var G3 = [255, 126, 107];
function lerp(a2, b2, t2) {
  return Math.round(a2 + (b2 - a2) * t2);
}
function gradientText(s2) {
  if (!COLOR) return s2;
  const chars = [...s2];
  const n2 = Math.max(chars.length - 1, 1);
  return chars.map((ch, i2) => {
    const t2 = i2 / n2;
    return rgb(lerp(G1[0], G3[0], t2), lerp(G1[1], G3[1], t2), lerp(G1[2], G3[2], t2), ch);
  }).join("");
}
var c2 = {
  bold: (s2) => wrap("1", s2),
  dim: (s2) => wrap("2", s2),
  violet: (s2) => rgb(157, 144, 255, s2),
  orchid: (s2) => rgb(217, 139, 214, s2),
  coral: (s2) => rgb(255, 156, 140, s2),
  blue: (s2) => rgb(135, 183, 255, s2),
  ok: (s2) => rgb(157, 144, 255, s2),
  muted: (s2) => rgb(139, 145, 164, s2)
};
function confidenceChip(conf) {
  switch (conf) {
    case "high":
      return c2.violet("[high]");
    case "inferred":
      return c2.orchid("[infr]");
    case "flagged":
      return c2.coral("[flag]");
  }
}
function kindLabel(type) {
  switch (type) {
    case "command":
      return c2.violet(type);
    case "loop":
      return c2.coral(type);
    case "hook":
      return c2.blue(type);
  }
}
function banner(version) {
  return `${gradientText("gradient")} ${c2.dim(`\xB7 analysis engine v${version}`)}`;
}
...
function defaultOpenLog(path) {
  mkdirSync(join13(path, ".."), { recursive: true });
  return openSync(path, "a");
}
function spawnDetached(args, projectDir, deps = {}) {
  const spawn2 = deps.spawn ?? realSpawn;
  const logPath = join13(gradientDir(projectDir), "last-scan.log");
...
var VERSION2 = true ? "0.1.0" : require2("../package.json").version;

// src/cli.ts
var HELP = `gradient \u2014 turn repeated Claude Code workflows into artifacts
...
function sinceDays(flag) {
  ...
  const m2 = /^(\d+)d?$/.exec(flag.trim());
  return m2 ? Number(m2[1]) : void 0;
}
async function main(argv, io = {}) {
  const log = io.log ?? ((s2) => process.stdout.write(s2 + "\n"));
...
${HELP}`);
  ...
  try {
    switch (command) {
      case "init": {
        const r2 = await init2({ installSkill: !flags["no-skill"], sessionScan: !!flags["session-scan"], projectDir });
        log(banner(VERSION2));
        log(
          `${c2.muted("backend:")} ${r2.backend}
${c2.muted("config:")} ${r2.configPath}
${c2.muted("skill installed:")} ${r2.skillInstalled}
${c2.muted("session-start scan:")} ${r2.sessionScanInstalled}`
        );
        return 0;
      }
      case "scan": {
        if (flags.detach) {
          const passthrough = argv.slice(1).filter((a2) => a2 !== "--detach");
        ...
        for (const s2 of out) {
          log(
            `  ${confidenceChip(s2.confidence)} ${c2.bold(s2.name)}  ${c2.muted(s2.title)}  ${c2.dim(`(seen ${s2.evidence.count}\xD7)`)}`
          );
          if (isNudge(s2)) {
            log(`      ${c2.dim("tip: this is what autopilot automates \u2192")} ${c2.violet("gradient autopilot nudge")}`);
          }
        }
        log(`
${c2.dim("Next:")} ${c2.violet("gradient review")}`);
        return 0;
      }
      case "review": {
        if (flags.json) {
        ...
        log(`
${c2.ok(`applied ${applied.length} suggestion(s).`)}`);
        for (const a2 of applied) {
          if (a2.printed) log(`  ${c2.dim("run:")} ${a2.printed}`);
        }
        return 0;
      }
      case "apply": {
        ...
        for (const a2 of applied) {
          log(a2.written ? `${c2.ok("wrote")} ${c2.muted(a2.written)}` : `${c2.dim("run:")} ${a2.printed}`);
        }
        return 0;
      }
      case "explain": {
        const s2 = await explain(projectDir, positionals[0] ?? "");
        if (!s2) {
          log(c2.coral(`no suggestion matching: ${positionals[0] ?? "(none given)"}`));
          return 1;
        }
        log(`${confidenceChip(s2.confidence)} ${c2.bold(s2.name)}  ${c2.muted(s2.title)}`);
        log(c2.dim(s2.rationale));
        log(c2.dim(`seen ${s2.evidence.count}\xD7 across ${s2.evidence.sessions} sessions`));
        for (const ex of s2.examples ?? []) log(`  ${c2.muted("\xB7")} ${ex}`);
        return 0;
      }
      case "list": {
        for (const e2 of await list(projectDir)) {
          log(`  ${c2.bold(e2.name)}	${kindLabel(e2.type)}	${c2.muted(e2.path || "(printed)")}	${c2.dim(e2.createdAt)}`);
        }
        return 0;
      }
      case "remove": {
        const ok = await remove(projectDir, positionals[0]);
        log(ok ? `${c2.ok("removed")} ${positionals[0]}` : c2.coral(`no such artifact: ${positionals[0]}`));
        return ok ? 0 : 1;
      }
      case "stats": {
        log(banner(VERSION2));
        const r2 = await stats(projectDir);
        log(c2.dim(`coverage: ${r2.covered}/${r2.total} patterns automated (${r2.coveragePct}%)`));
        log(c2.dim(`session-start scan: ${r2.sessionScanEnabled ? "on" : "off"}`));
        for (const p2 of r2.patterns) {
          log(`  ${confidenceChip(p2.confidence)} ${c2.bold(p2.name)}  ${c2.dim(`(seen ${p2.count}\xD7 \xB7 ${p2.sessions} sessions)`)}  ${p2.covered ? c2.ok("\u2713 automated") : c2.muted("\u2014")}`);
        }
        return 0;
      }
      case "checkpoint": {
      ...
      case "autopilot": {
        ...
        if (arg === "off" || arg === "nudge" || arg === "full") {
          const r2 = await setAutopilotMode(arg, projectDir);
          log(banner(VERSION2));
          log(`${c2.muted("autopilot:")} ${c2.bold(r2.mode)}`);
          log(
            r2.hookInstalled ? `${c2.ok("Stop hook installed")} ${c2.muted(r2.settingsPath)}` : `${c2.muted("Stop hook removed:")} ${r2.settingsPath}`
          );
          return 0;
        }
        if (arg !== "status") {
          log(c2.coral(`unknown autopilot mode: ${arg} (use off|nudge|full|status)`));
          return 2;
        }
        const s2 = await autopilotStatus(projectDir);
        log(banner(VERSION2));
        log(`${c2.muted("mode:")} ${c2.bold(s2.mode)}${s2.effectiveMode !== s2.mode ? c2.dim(` \u2192 ${s2.effectiveMode} here (clamped by project gradient.md)`) : ""}`);
        log(`${c2.muted("budget:")} ${s2.budget} auto-responses/session${s2.effectiveBudget !== s2.budget ? c2.dim(` \u2192 ${s2.effectiveBudget} here (clamped by project gradient.md)`) : ""}`);
        log(`${c2.muted("gradient.md:")} ${s2.playbookPath}${s2.playbookExists ? "" : c2.dim(" (not yet generated \u2014 run gradient scan)")}`);
        log(
          `${c2.muted("project gradient.md:")} ${s2.projectPlaybookExists ? s2.projectPlaybookPath + (s2.projectMalformed ? c2.coral(" (malformed \u2014 autopilot off here)") : "") : c2.dim("none in this repo")}`
        );
        log(`${c2.muted("stop hook here:")} ${s2.hookInstalled ? c2.ok("installed") : "not installed"}`);
        for (const e2 of s2.recent) {
          log(`  ${c2.dim(e2.ts)} ${e2.action === "continue" ? c2.ok("continued") : c2.muted("stood down")}  ${c2.dim(e2.why)}`);
        }
        return 0;
      }
      case "respond": {
        try {
          const input = await readStdin();
          const r2 = await respond(input);
          if (r2.decision === "block") log(JSON.stringify({ decision: "block", reason: r2.reason }));
        } catch {
        }
        return 0;
      }
      default:
        log(`${c2.coral(`unknown command: ${command}`)}
