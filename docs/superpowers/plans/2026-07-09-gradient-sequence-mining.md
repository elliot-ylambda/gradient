...
**Goal:** Recurring prompt chains ("review the spec" → "write the plan") become multi-step skill suggestions and deterministic Workflows lines in the global `gradient.md` mined region. Spec: `docs/superpowers/specs/2026-07-09-gradient-sequence-mining-design.md`.
...
- **Constants (spec §2, pinned here):** `SEQ_MIN_COUNT = 3`, `SEQ_MIN_SESSIONS = 2`, `SEQ_MAX_BIGRAMS = 2000`, `PLAYBOOK_MAX_CHAINS = 5`, detect-window share `Math.ceil(window / 4)`.
- One merge pass only: chains are at most 3 steps (two merged bigrams) in v1.
...
| File | Responsibility |
|------|----------------|
| `cli/src/core/sequence.ts` (create) | `NUDGE_PROMPT_RE`, `ChainFinding`, `mineSequences` — pure pair counting + one merge pass |
| `cli/src/core/types.ts` (modify) | `Candidate.kind` gains `"sequence"` |
| `cli/src/core/playbook.ts` (modify) | chain lines in the existing "My workflows (mined)" subsection |
| `cli/src/commands/scan.ts` (modify) | wire `mineSequences` → detect window + playbook; cap logging |
