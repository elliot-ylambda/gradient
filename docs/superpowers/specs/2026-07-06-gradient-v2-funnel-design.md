...
- `core/paste.ts`: pre-cluster classifier for `"human"` prompts with
  ...
  Extraction key = first non-empty command-like or error-like line, truncated
  (~80 chars) — pasted
...
- `detect.ts` prompt turns paste candidates into a **skill** — "run `<cmd>`
  yourself, then fix what fails" (kills the paste round-trip entirely).
...
- **Continuity pack** (the productized `/sum`): an explicit opt-in manager,
  ...
  `gradient recap`, which prints `progress.md` to stdout so it re-enters
  context after compaction/resume. `checkpoint` captures a redacted recent
  conversation tail (via `core/tail.ts`) in addition to recent user intents.
  Nothing is installed until the user runs `continuity on`; `off` removes only
  these two hooks. This is the same explicit consent model as recall/autopilot
  and avoids asking the detect LLM to coordinate a paired artifact.
